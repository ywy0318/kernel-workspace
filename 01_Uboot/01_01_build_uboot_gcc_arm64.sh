#!/bin/bash
# 脚本名：01_01_build_uboot_gcc_arm64.sh
# GCC aarch64交叉编译U-Boot
# 日志文件定义，每次编译生成带时间戳的日志
LOG_FILE="./uboot_gcc_build_$(date +%Y%m%d_%H%M%S).log"
# 重定向所有输出到日志，同时打印终端
...
# GCC产物输出目录
OUTPUT_DIR="${CUR_DIR}/output-gcc"

# 创建输出目录
mkdir -p "${OUTPUT_DIR}" || { echo -e "\033[0;31m错误：创建GCC输出目录失败 ${OUTPUT_DIR}\033[0m"; exit 1; }
...
START_TIME=$(date +%s)
echo -e "\n=================================================="
echo "【GCC版本】U-Boot 编译开始时间：$(date '+%Y-%m-%d %H:%M:%S')"
...
# 编译
echo "[3/4] 开始编译 U-Boot (arm64 GCC)..."
make -j$(nproc)
...
# 最终状态提示与编译耗时统计
echo -e "\n=================================================="
echo "【GCC版本】U-Boot 编译结束时间：$(date '+%Y-%m-%d %H:%M:%S')"
echo "编译总耗时：${ELAPSED_MIN}分${ELAPSED_REMAIN_SEC}秒"
echo "=================================================="

if [ $ALL_SUCCESS -eq 1 ]; then
    echo -e "\033[0;32m✅ GCC U-Boot 编译全部完成，产物已拷贝至 ${OUTPUT_DIR}\033[0m"
else
    echo -e "\033[0;31m❌ GCC U-Boot 编译失败，必需产物缺失\033[0m"
    exit 1
fi
