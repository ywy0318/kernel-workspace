#!/bin/bash
# 04_09_07_qemu_uboot_gcc_kernel_gcc.sh
# 配套04_07预处理脚本｜方案2：双virtio磁盘，内核独立磁盘
# 组合：GCC U-Boot + GCC Kernel
set -e

echo "============================================="
echo "04_09_07_qemu_uboot_gcc_kernel_gcc.sh | GCC U-Boot + GCC Kernel QEMU(双磁盘方案2)"
echo "============================================="
...
# 04_07脚本生成产物
ROOTFS_IMG="./build_uboot_kernel_pair_ubgcc_kerngcc/rootfs.ext4"
KERNEL_DISK="./build_uboot_kernel_pair_ubgcc_kerngcc/kernel_Image.raw"

# 文件完整性校验
if [ ! -f "${UBOOT_BIN}" ];then
    echo "❌ 错误：GCC编译U-Boot固件不存在 ${UBOOT_BIN}"
    exit 1
fi
...
# 双virtio磁盘配置，内核盘readonly只读，无-serial stdio
qemu-system-aarch64 \
