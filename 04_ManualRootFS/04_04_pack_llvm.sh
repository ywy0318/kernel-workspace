...
# ===================== 路径常量 统一配置 =====================
# UBoot 输出目录（uboot不分gcc/llvm，共用一套）
UBOOT_BUILD_PATH="/home/ubuntu/workspace/01_Uboot/u-boot-v2025.04"
# LLVM内核编译输出目录
KERNEL_LLVM_PATH="/home/ubuntu/workspace/02_LinuxKernel/build-llvm"
# rootfs镜像路径（当前脚本同目录）
ROOTFS_IMG="${PWD}/rootfs.ext4"
# 输出存放目录
OUTPUT_DIR="${PWD}/output_llvm"
# 最终打包文件名
PACK_NAME="firmware_llvm.img"
# ==========================================================

echo "============================================="
echo "LLVM 全套固件打包脚本启动"
echo "============================================="
...
# LLVM内核Image
if [ ! -f "${KERNEL_LLVM_PATH}/Image" ]; then
    echo "ERROR: 缺失 LLVM 内核 Image: ${KERNEL_LLVM_PATH}/Image"
    exit 1
fi
# rootfs镜像
...
# 拷贝LLVM内核镜像
echo "[4/6] 拷贝 LLVM 编译 Linux 内核 Image"
cp "${KERNEL_LLVM_PATH}/Image" "${OUTPUT_DIR}/kernel/"
echo "llvm kernel 文件拷贝完成"
echo
...
# 打包成 .img tar归档文件
echo "[6/6] 将 output_llvm 目录打包为 ${PACK_NAME}"
rm -f "${PACK_NAME}"
tar -cf "${PACK_NAME}" -C "${PWD}" output_llvm
